...
//! (see [`Host`]). Two reasons, both load-bearing:
//!
//! - **Apartment pinning.** [`CommandCache`] keeps activated
...
use windows::Win32::UI::WindowsAndMessaging::{
    DestroyIcon, DispatchMessageW, GetIconInfo, HICON, ICONINFO, MSG, MsgWaitForMultipleObjects,
    PM_REMOVE, PeekMessageW, QS_ALLINPUT, TranslateMessage,
};
...
fn build_menu(
    ...
    for (clsid, app) in candidates {
        ...
        let icon_data_url =
            take_pwstr(unsafe { command.GetIcon(&array) }.ok()).and_then(|spec| cache.icon(&spec));
...
fn shell_item_array(paths: &[String]) -> Result<IShellItemArray, FileSystemError> {
    ...
    array.map_err(|error| FileSystemError::Internal(format!("fs.shell_command_selection: {error}")))
}
...
<Package xmlns="http://schemas.microsoft.com/appx/manifest/foundation/windows10"
  ...
  <desktop4:FileExplorerContextMenus>
    ...
    fn warming_leaves_the_next_list_on_the_fast_path() {
        ...
        let started = std::time::Instant::now();
        let items =
            tauri::async_runtime::block_on(list(paths, primary)).expect("listing shell commands");
